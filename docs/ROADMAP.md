...
- Continue hardening the now-default `masked_depth_detail_blend` relief path: geometry-only proof cleanup, semantic depth, contour-smoothed subject mask, in-mask detail blend from the deterministic lithophane source, guided-filter bas-relief compression, nose-aware portrait shaping, and the existing closed-mesh STL/GLB generator.
- Keep face-aware portrait tuning inside the server-side print-file generator, starting with local/on-service face detection or landmarks to build soft region masks. Use these masks to preserve larger facial forms, make the nose read as a protruding form, and damp harsh deterministic detail around eyes, teeth, mouth, and skin texture; defer a separate external face API until product-flow testing proves local detection is insufficient.
