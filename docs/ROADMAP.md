...
- ⚪ App-host indexing guard — when the auth-gated product app is exposed at `app.3dprintyou.com`, noindex it domain-wide because it is thin, duplicate-risk, and not the SEO surface. Add app-level `X-Robots-Tag: noindex, nofollow, noarchive` plus Next metadata robots, add a Cloudflare response-header guard for only `app.3dprintyou.com`, keep it out of sitemaps, and verify with `curl -I` and Search Console. Do not apply this to the public SEO apex `3dprintyou.com` or the webhook host `api.3dprintyou.com`.
