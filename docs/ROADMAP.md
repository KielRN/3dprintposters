...
- 🟢 Figurine style families — two approved tracks as of 2026-07-03: **Chibi** via the Creative Lab Figure pipeline (Experiment 011, best fed a stylized 2D image) and **faithful identity** via direct Multi-Image-to-3D with a single Nano Banana Pro 2K identity-locked front image (Experiment 019a). Recipes are in `DECISIONS.md`. Chibi is wired into the customer workflow config and UI as of 2026-07-03, and the template-face-swap chibi flow (admin template + customer face -> Meshy concept gate -> build) was validated end to end and user-approved on 2026-07-04; wiring the faithful direct Multi-Image-to-3D style is the remaining follow-up.
