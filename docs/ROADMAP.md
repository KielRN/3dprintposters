...
- AI proof generation should eventually return or imply style/surface metadata that the print-file generator can consume: surface regions such as `smooth_skin`, `smooth_body`, `smooth_fabric`, `flat_background`, `raised_text`, `raised_logo`, `panel_line`, `hair_texture`, or other printable texture classes.
