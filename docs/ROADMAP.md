...
- Add face-aware portrait tuning inside the server-side print-file generator, starting with local/on-service face detection or landmarks to build soft region masks. Use these masks to preserve larger facial forms while damping harsh deterministic detail around eyes, teeth, mouth, and skin texture; defer a separate external face API until product-flow testing proves local detection is insufficient.
