...
- `MESHY_WEBHOOK_URL` and `MESHY_WEBHOOK_SECRET` are placeholders for the future Meshy webhook receiver. Meshy webhook creation is currently configured in the Meshy web app API settings page, not through a documented REST endpoint.
...
- Webhooks require an HTTPS URL. Prefer a Cloudflare Worker or Cloudflare-routed endpoint such as `https://api.3dprintyou.com/webhooks/meshy`; use local polling until that endpoint exists.
...
- Current local Cloudflare credentials need refresh: `CLOUDFLARE_API_TOKEN` is present in root `.env`, but token verification returned `401` on 2026-05-23 and zone lookup returned `403`.
