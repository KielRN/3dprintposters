...
Current workflow source of truth: implemented figurine/customer/operator workflow details now live in `docs/Workflows/figurine-and-operator-workflows.md`, and current style contracts live in `docs/Workflows/figurine-style-workflow-contracts.md`. Keep this file as PrintU-inspired planning context and baseline UX/job-contract history, not as the current style-by-style workflow contract.
...
Immediate implementation target: keep the Creative Lab figurine path honest while the public workflow details are maintained in `docs/Workflows/`. Current Chibi behavior is summarized in `docs/Workflows/figurine-style-workflow-contracts.md`: `/start` style gallery -> `/start/{styleId}` upload/project page -> style-specific proof or cleanup stage -> Meshy Creative Lab prototype concept -> customer approval -> checkout -> post-payment Meshy build for operator/support review. Local emulator runs can still mirror provider assets for Blender/slicer inspection.
...
- Chibi: small body, cute stylized proportions, soft features. Current public Chibi details live in `docs/Workflows/figurine-style-workflow-contracts.md`.
...
- Store concept metadata: source image path, style, posture, prompt contract version, per-style reference image count/IDs when used, provider, moderation status, and chosen/approved state.
...
9. In the current funded-build flow, approval records the concept and unlocks checkout.
10. Stripe payment queues the provider 3D build, or manual studio-review release queues it for fallback orders.
11. Provider GLBs, print-readiness, and fulfillment state remain operator/support concerns.

The current Chibi paths do not show multiple customer proof options. They use style-specific pre-Meshy input preparation plus a Meshy prototype concept gate; see `docs/Workflows/figurine-style-workflow-contracts.md`.
