...
- [x] Build the deterministic name-on-base service in `services/print-file-generator`. `app/figurine_name_base.py` validates sign names (12-character cap), composes DejaVu Sans Bold lettering into the `figurine-square-v1` base as one watertight mesh, and exports STL/3MF/preview GLB through `POST /v1/figurine/named-base` and the `scripts/compose_named_base.py` CLI. Asset manifest remains `pending-approval` until Elliott's review in `human-tasks/open/2026-06-10-review-named-base-service.md`.
- [x] Wire the base name sign into the figurine job page. After the Creative Lab GLB preview renders, a base sign panel lets the customer toggle the sign, enter a validated name, and call `updateFigurineBaseConfig`; the job page then shows `baseConfig`/`figurineNamedBase` state and an orbitable named-base preview GLB.
