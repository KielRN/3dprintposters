...
- Added the first deployed full product-app candidate at `https://3dprintposters-production.up.railway.app` in the existing Railway project. The repo now builds the `apps/web` Next.js app as a standalone server, copies its static/public assets into the standalone bundle, exposes `/api/health`, and starts through the root `railway.toml` with `HOSTNAME=0.0.0.0` on Railway's injected port.
...
- Verified the Functions TypeScript build, the Railway standalone build/start path, and a successful Railway healthcheck rollout after correcting the server bind address.
- Verified the active Railway release returns `200` for `/` and `{ "status": "ok" }` for `/api/health`, with the expected `X-Robots-Tag` header.
- Verified Firebase Auth lists the Railway hostname, Firebase Storage CORS includes the exact HTTPS origin, and deployed `createCheckoutSession` is active with the current `PUBLIC_APP_URL` secret version.

## [Unreleased] - 2026-07-12

### Added

- Added three repo-scoped figurine workflow skills under `.agents/skills/`: `$edit-figurine-workflow-prompts` for Vertex/Gemini prompt and preset changes, `$debug-figurine-workflow` for tracing failed or stalled figurine jobs, and `$add-figurine-workflow-style` for adding or mirroring workflow styles. Updated the agent/source-map docs to point future agents at the skills and their helper scripts.
- Added customer-owned hero removal for the `/start/[styleId]` "Your heroes" grid. Deletable unpaid cards now show a trash action that calls an owner-checked `deleteOwnJob` Function, soft-deletes the job from the customer grid, and refuses active checkout or paid/fulfillment jobs so order history stays intact.
- Added personal studio review recovery for figurine generation terminal states. `createGenerationJob` now records durable `generationState` progress, runs with 512 MiB so Vertex proof bytes and the provider handoff cannot exceed the old 256 MiB container limit, and no longer resizes Creative Lab input inside Functions. `reconcileStaleGenerationJobs` moves abandoned figurine generations into a terminal studio-review state, and the customer job page calls the same server-validated reconciliation path so stale jobs also recover during Functions-emulator testing or before the next scheduler tick. `/jobs/[jobId]/manual-checkout` offers a disclosed manual-review Stripe path, and `createFallbackFigurineCheckoutSession` writes `fulfillmentMode: "manual_proof_required"` orders without weakening the normal proof-approved checkout guard. The Stripe webhook keeps paid manual-review orders in support/operator review and skips automatic `figurineBuild` queueing until an operator releases a reviewed concept. Added `repair:stale-generation-jobs` for tightly scoped repair of stranded dev/live jobs.

### Changed

- Compressed `docs/Workflows/` from separate per-style pages into `figurine-and-operator-workflows.md` plus the new `figurine-style-workflow-contracts.md` runtime matrix. Updated AGENTS, project state, decisions, roadmap/source maps, and repo-scoped workflow skills to point at the two-doc structure and the current funded-build checkout boundary.
...
- Workflow controls now offer a reusable `Template face swap-female (single concept)` prompt preset for any style. It saves as the existing `template_face_swap` mode, but fills the Vertex prompt with instructions for a `full-body collectible action figure` using `smooth toy material`; the Chibi female seed script also writes the current `Chibi heroic fantasy female` label when refreshing `adminConfig/figurineWorkflow`.
...
- Added preview-only figurine UI support for Storage-backed original textured Creative Lab GLBs. Jobs with `productType: "figurine"` and `figurinePreview.previewGlb` rendered a color 3D model preview marked as not print-ready. At that point figurine checkout was still blocked until a future print-ready fulfillment path; this was later superseded by the funded-build flow.
