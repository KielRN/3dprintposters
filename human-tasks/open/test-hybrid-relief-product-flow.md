...
6. Confirm the generated metadata reports `normalized_width_px: 280`, `normalized_height_px: 392`, `triangle_count` below 500,000, and `binary_stl_bytes` below 25,000,000.
7. Confirm the generated `preview.glb` is no longer neutral gray and uses image-derived color while `model.stl` under `.tmp/print-files/{uid}/{jobId}` still represents a 5.5in x 7.5in physical object with a 5in x 7in image relief window and shaped 1/4in border/frame.
8. Spot-check the mirrored color package files under `.tmp/print-files/{uid}/{jobId}/full-color`: `print-package.3mf`, `model.obj`, `model.mtl`, `texture.png`, `model.wrl`, and `model.ply`.
9. Spot-check the mirrored filament painting files under `.tmp/print-files/{uid}/{jobId}/filament-painting`: `palette.json`, `layer-swaps.txt`, `print-settings.json`, and `preview.png`.
10. Confirm checkout unlocks only after the hybrid print-file artifacts are generated.
11. If the flow fails, package files do not open, or the relief/color quality is not acceptable, capture the failure as the next AI developer fix target.
...
- The generated artifact metadata reports `normalized_width_px: 280`, `normalized_height_px: 392`, `width_mm: 139.7`, `height_mm: 190.5`, `image_window_width_mm: 127.0`, `image_window_height_mm: 177.8`, `border_mm: 6.35`, `full_color_package`, and `filament_painting`.
...
- Notes about whether the larger 280px GLB preview remains responsive enough for product review.
