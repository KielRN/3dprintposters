...
def test_masked_depth_detail_blend_adds_detail_only_inside_subject(
    ...
    def fake_infer_depth(img: Image.Image) -> np.ndarray:
        row = np.linspace(0.2, 0.8, 24, dtype=np.float32)
        return np.tile(row, (24, 1))
    ...
    assert _mean_adjacent_delta(subject) > _mean_adjacent_delta(background) * 2.0
...
def test_masked_depth_detail_blend_detail_source_changes_subject_output(
    ...
    subject_diff = np.mean(np.abs(lithophane.values[4:20, 4:20] - posterized.values[4:20, 4:20]))
    background_diff = np.mean(np.abs(lithophane.values[0:4, :] - posterized.values[0:4, :]))
