...
- Current relief-quality direction is surface-intent aware generation: the hybrid provider now infers v1 smooth/crisp/texture masks so skin, scalp, neck, ears, hands, simple clothing, and backgrounds remain smooth by default, crisp text/logos/graphic edges stay raised, and shallow material texture is enabled only from explicit proof-generation or human override metadata.
...
3. Add region-aware roughness quality gates or debug metrics so smooth skin/body/fabric/background regions warn when high-frequency geometry is still too strong.
