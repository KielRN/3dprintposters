...
Development posture: build toward the intended final product behavior first. Once a product direction has been reviewed and chosen, wire it into the real user workflow instead of leaving it as opt-in experiment code. Prefer loud failures during testing over quiet lower-quality fallback behavior; when human testing finds a failure, fix that failure directly.
...
- `approveGeneratedImage` sets `approvedImagePath`, calls `PRINT_FILE_GENERATOR_URL`, generates baseline print-file artifacts under `print-files/{uid}/{jobId}`, and persists `printFileStatus`, `printFileOutputPrefix`, `printFileArtifacts`, `printability`, and `printFileAudit` on the job.
...
- The current print-file implementation path is hybrid closed relief generation: validated image input up to 4,000,000 decoded pixels by default, 5:7 crop/pad, `masked_depth_detail_blend` heightmap generation, Depth Anything V2 semantic depth, SegFormer subject masking, `lithophane_baseline` in-mask detail, guided-filter bas-relief compression, closed 127mm x 177.8mm mesh with top surface/base/sidewalls, binary STL, neutral-material GLB preview, heightmap PNG, metadata JSON, and printability checks.
...
- Experiment 1 (deterministic) outcome: `posterized_luminance`, `continuous_luminance`, and `lithophane_baseline` remain useful references, but `posterized_luminance` is no longer the checkout default.
- Height provider policy is now executable in `services/print-file-generator/app/provider_policy.py`. `metadata.json` includes `height_provider_policy`, `height_provider_fallback_only`, `height_provider_target_quality_path`, and `height_provider_checkout_default_allowed`; deterministic brightness-to-height providers also add a printability warning that they are not the target production-quality relief path.
...
- Experiment 4 outcome: `segformer_masked_depth` (renamed from `sam_masked_depth` on 2026-05-09 to match the actual SegFormer/ADE20K-via-HF-Inference implementation) passes all calibrated quality gates after the bas-relief swap and is the foundation for the hybrid provider. Historical `sam_masked_depth/` artifacts under `.tmp/experiments/experiment_4/` are retained for audit.
- Hybrid provider status: `masked_depth_detail_blend` is the default product relief provider. It combines Depth Anything semantic depth, SegFormer subject masking, subject-only high-frequency detail from a deterministic provider, guided-filter bas-relief compression, and the existing closed STL/GLB generator. The default in-mask detail source is `lithophane_baseline`.
- Hybrid detail-source comparison (2026-05-10): both `masked_depth_detail_blend__lithophane_baseline` and `masked_depth_detail_blend__posterized_luminance` passed the calibrated gates on the two canonical inputs. Lithophane detail had better composition metric values on both inputs (`-0.1757` vs `-0.1978` portrait, `0.0721` vs `0.0643` Gemini), while posterized detail had lower high-frequency noise and mask ridge. Product decision on 2026-05-11: use `masked_depth_detail_blend` with `lithophane_baseline` detail source in the web approval flow.
...
- Production provider registry: each AI role lives behind a typed `*Provider` Protocol with a `*Chain` that records `ProviderAudit` (succeeded provider, attempted chain, reason, model version). Scaffolding under [services/print-file-generator/app/providers/](services/print-file-generator/app/providers/). Roles wired today: `SubjectSegmentationProvider` (concrete: HF Inference SegFormer; stubs: Vertex Vision, Cloudflare gateway) and `MonocularDepthProvider` (concrete: local Depth Anything V2 - dev-only; stubs: HF Inference Depth Anything, Vertex). `metadata.json` now includes `provider_audit` and `segmentation_status` when the selected provider uses depth and/or segmentation. Functions also copy the audit into `jobs/{jobId}.printFileAudit`, `jobs/{jobId}/audit/printFileGeneration`, and checkout order records.
...
- AI Gateway is not an MVP dependency. Start with direct Vertex/Gemini calls from server runtimes, and add Cloudflare AI Gateway later if provider comparison, centralized AI observability, rate limits, or retries become important.
...
- ~~Whether and when Cloudflare AI Gateway should be added after direct Vertex/Gemini MVP integration.~~ Settled 2026-05-09: Cloudflare AI Gateway is the unified observability/rate-limit pane for the provider registry, not an MVP-only afterthought. Wire roles through it as gateway-served implementations land.
- Whether users can edit depth/relief settings before checkout.
- ~~Whether a source-photo proof substitute is needed for provider outages or if generation failures should stay hard failures.~~ Settled 2026-05-11: favor hard failures during testing and fix the failed production path directly.
...
- ~~Build the hybrid `masked_depth_detail_blend` provider that combines semantic depth, subject masking, in-mask detail blend, and the new guided-filter compression.~~ Completed and promoted to the default web approval flow on 2026-05-11 with `lithophane_baseline` detail source.
